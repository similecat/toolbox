...
def _supports_cuda_arch(device_index: int) -> bool:
    """Check if the current PyTorch build supports the selected CUDA device arch."""
    try:
        major, minor = torch.cuda.get_device_capability(device_index)
        required_arch = f"sm_{major}{minor}"
        supported_arches = torch.cuda.get_arch_list()
        return required_arch in supported_arches
    except Exception:
        # If capability probing fails, allow later runtime checks to decide.
        return True
...
def _cuda_runtime_usable() -> bool:
    ...
    try:
        ...
        if not _supports_cuda_arch(device_index):
            major, minor = torch.cuda.get_device_capability(device_index)
            print(
                f"Warning: CUDA arch sm_{major}{minor} is not supported by the installed PyTorch build. "
                "Falling back to CPU."
            )
            return False

        # Trigger a tiny kernel launch to verify runtime compatibility.
...
def main():
    ...
    if args.device == "cuda" and not _cuda_runtime_usable():
        args.device = "cpu"
